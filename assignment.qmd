...
# For impeachment - 209, against impeachment - 26
impeach %>%
...
```{r}
# Same thing as above but also group by GDP
impeach %>%
...
```{r}
# Replace GDP with college grads
impeach %>%
  group_by(for_impeachment,
...
```{r}
# Take average of pct_bachelors for yes/no responses
impeach %>%
  group_by(for_impeachment) %>%
  summarise(grad_average_percent = mean(pct_bachelors))
...
```{r}
# Replace with trump_percent
impeach %>%
  group_by(for_impeachment) %>%
  summarise(trump_average_percent = mean(trump_percent))
...
```{r}
# Breakdown by y/no to impeachment and average income
impeach %>%
  group_by(for_impeachment) %>%
  summarise(income_average = mean(median_income))
...
```{r}
# summarise by counting # of each month
impeach2 %>%
